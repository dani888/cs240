...
void freef(char *p)
{
  ...
  if (p != NULL) {
    ...
    struct blockl *blp = (struct blockl *)(p - TAGSIZE);
    if(blp->tag == FREETAG){return;}
    blp->tag = FREETAG;
    char *cursor = p;
    cursor += blp->size - (2 * TAGSIZE);
    struct blockr *brp = (struct blockr *)cursor;
    brp->tag = FREETAG;
    cursor = (p - (TAGSIZE * 2));
    struct blockr *brpls = (struct blockr *)cursor;
    /* brpls -= 1; */
    struct blockl *blpNew;
    if (brpls->tag == FREETAG) {
      struct blockl *blpls = (struct blockl *)(cursor - brpls->size + TAGSIZE);
      coalesce(blpls, blp);
      blpNew = blpls;
    }
    else {
    ...
    cursor += TAGSIZE + blp->size;
    struct blockl *blprs = (struct blockl *)cursor;
    if (blprs->tag == FREETAG) {
      unchain(blprs);
      coalesce(blpNew, blprs);
    }
  }
}
